...
VERSION_DIR_RE = re.compile(r"^v\d+(?:[._-]\d+)*$")
...
CONTRACT_ROOT_CANDIDATES = ("contracts", "docs/contracts")
...
def _contract_roots() -> list[Path]:
    roots: list[Path] = []
    for rel in CONTRACT_ROOT_CANDIDATES:
        path = REPO_ROOT / rel
        if path.exists():
            assert path.is_dir(), f"contract path must be a directory: {rel}"
            roots.append(path)
    return roots


def _has_versioned_contract_artifact(root: Path) -> bool:
    for path in root.rglob("*"):
        if path.is_dir() and VERSION_DIR_RE.match(path.name):
            return True
        if path.is_file() and any(VERSION_DIR_RE.match(part) for part in path.parts):
            return True
    return False
...
def test_decisions_directory_exists() -> None:
    assert (REPO_ROOT / "docs" / "decisions").is_dir(), "docs/decisions/ is required"

def test_enterprise_baseline_doc_exists() -> None:
    assert (REPO_ROOT / "docs" / "ENTERPRISE_TRUST_BASELINE.md").is_file(), "docs/ENTERPRISE_TRUST_BASELINE.md is required"



def test_contract_roots_are_directories_when_present() -> None:
    _contract_roots()


def test_versioned_contract_artifacts_when_contracts_present() -> None:
    roots = _contract_roots()
    if not roots:
        return
    assert any(_has_versioned_contract_artifact(root) for root in roots), (
        "contract roots exist but no versioned contract artifacts were found"
    )
