...
- [7fcab1ba-2af2-498d-b712-d6c79520fd6e] Friday direct chat now auto-routes code prompts to coder and injects repo file context end-to-end
  ...
  explanation: path=v2_hybrid_recall sources=['body', 'evidence', 'reinforcement_state', 'summary', 'title', 'vector'] tokens=['code', 'context', 'direct', 'friday', 'only', 'read', 'repo', 'route']
- [ea8d2c51-9a13-4736-8b12-45c068c6a783] Friday bridge and tools now expose read-only /mnt/data workspace access
  kind: decision status: active stage: primary_retrieval reason: query_match
  score: 149.207533 rank: 2
  ...
  explanation: path=v2_hybrid_recall sources=['body', 'evidence', 'reinforcement_state', 'summary', 'title', 'vector'] tokens=['direct', 'friday', 'only', 'read', 'repo', 'workspace']
- [849362c5-ed42-43a2-9084-897ecf2c7de1] Friday root now has explicit Mimir repo-cognition entrypoints separated from runtime memory
  ...
  explanation: path=v2_hybrid_recall sources=['body', 'evidence', 'reinforcement_state', 'summary', 'title', 'vector'] tokens=['boundary', 'cognition', 'context', 'friday', 'mimir', 'repo']
...
- [ed79d474-0488-4603-8a70-5a5d6a78e4bc] Friday runtime now applies pre-dispatch token budgeting with truthful context-window errors
  kind: decision status: active stage: recent_in_scope_supplement reason: recent_query_primary_domain_overlap
  score: 14.5 rank: None
  summary: Chat requests now estimate remaining context budget, trim old history when needed, cap completion tokens before LLM dispatch, and map context-limit failures to explicit 400 context_budget_exceeded instead of generic 502.
  body: Implemented max_tokens_override + context-limit classification in backend/core/llm.py and request fitting in backend/core/chat_engine.py. Added per-task eval identity isolation in evals/runner to prevent cross-case context growth. Runtime smoke on 18 previously failing generated variants moved from 14/18 HTTP 502 to 18/18 HTTP 200.
  evidence: /mnt/data/friday/evals/remediation/remediation_pass_v1_metrics.json; /mnt/data/friday/backend/core/chat_engine.py; /mnt/data/friday/backend/core/llm.py; /mnt/data/friday/evals/runner/run_corpus.py
  explanation: path=recent_in_scope_shadow_supplement sources=['query_tokens', 'title_summary_tokens', 'primary_result_domain_tokens'] tokens=['added', 'backend', 'case', 'chat', 'core', 'data', 'engine', 'explicit', 'friday', 'implemented', 'instead', 'request', 'runtime', 'when']
...
- [1585ad48-46dc-4272-b874-4190dbd2acf2] card=ed79d474-0488-4603-8a70-5a5d6a78e4bc type=file ref=/mnt/data/friday/evals/remediation/remediation_pass_v1_metrics.json
- [424b4b66-508d-4b05-9c17-bf4cc3edd460] card=ed79d474-0488-4603-8a70-5a5d6a78e4bc type=file ref=/mnt/data/friday/backend/core/chat_engine.py
- [620df070-2c29-4238-b0af-c8ee968587a1] card=ed79d474-0488-4603-8a70-5a5d6a78e4bc type=file ref=/mnt/data/friday/backend/core/llm.py
- [e16643d6-f04d-4471-b18e-c5e128dfe634] card=ed79d474-0488-4603-8a70-5a5d6a78e4bc type=file ref=/mnt/data/friday/evals/runner/run_corpus.py
...
- Continuity: Friday runtime now applies pre-dispatch token budgeting with truthful context-window errors - Chat requests now estimate remaining context budget, trim old history when needed, cap completion tokens before LLM dispatch, and map context-limit failures to explicit 400 context_budget_exceeded instead of generic 502.
